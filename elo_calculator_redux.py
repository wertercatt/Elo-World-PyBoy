...
from common_methods import Battle, List, Trainer, get_trainer_by_id, load_pickle
...
def greatest_diffs_cluster(elo_list, cluster_count=14):
    boundaries = sorted(
        list(range(len(elo_list) - 1)),
        key=lambda i: elo_list[i + 1] - elo_list[i],
        reverse=True,
    )
...
def battles_with_wasted_heal(battles: Iterable[Battle]):
    for battle in battles:
        if (
            "movie" not in battle.original_path
            and "POTION" in battle.turns[0].action.item
        ):
...
def main():
    ...
    for trainer in trainer_list:
        if (
            current_kde_tier < len(kde_boundaries)
            and trainer.lr_elo > kde_boundaries[current_kde_tier]
        ):
            current_kde_tier += 1
        if (
            current_diff_tier < len(greatest_diff_boundaries)
            and trainer.lr_elo > greatest_diff_boundaries[current_diff_tier]
        ):
            current_diff_tier += 1
        if (
            current_human_tier < len(human_boundaries)
            and trainer.lr_elo >= human_boundaries[current_human_tier] - 0.1
        ):
        ...
        print(
            "\t".join(
                (
                    ...
                    trainer.identifier,
                    "location",
                    trainer.location if trainer.location else "somewhere",
                    "party:",
                    ", ".join(
                        f"Lv. {mon.level} {mon.species}" for mon in trainer.party_mons
                    ),
                    "class:",
                    str(trainer.class_id),
                    "instance:",
                    str(trainer.instance_id),
                    "win count:",
                    str(win_count),
                    "lose count:",
                    str(lose_count),
                    ...
                    str(trainer.elo),
                    "kde tier:",
                    str(current_kde_tier),
                    "diff tier:",
                    str(current_diff_tier),
                )
            )
        )

    battle_list.sort(
        key=lambda b: b.losing_trainer.lr_elo - b.winning_trainer.lr_elo, reverse=True
    )

    lorelei_draws = [
        b
        for b in battle_list
        if b.player.class_id == 244 and b.winner.startswith("Draw")
    ]
    print("")
    ...
    if DO_POWERPOINT:
        ...
        for trainer in trainer_list:
            ...
            trainer.greatest_victory = max(
                victories, key=lambda b: b.losing_trainer.lr_elo
            )
            trainer.greatest_defeat = min(
                defeats, key=lambda b: b.winning_trainer.lr_elo
            )
        tier_names = [
            "F",
            "D-",
        ...
        for i, slide in enumerate(prs.slides):
            ...
            trainer_rank = 391 - i
            trainer_old_rank = 391 - next(
                i
                for i, t in enumerate(orig_battle_trainers)
                if t.identifier == trainer.identifier
            )
            ...
            for shape in slide.shapes:
                if shape.name == "win":
                    ...
                    shape.width = int(
                        (trainer.win_count + trainer.draw_count)
                        / battle_count
                        * lose_width
                    )
                elif shape.name == "tier":
                    shape.fill.fore_color.rgb = RGBColor.from_string(
                        tier_colors[trainer.tier]
                    )
                elif shape.name == "pic":
                    image, rid = shape.part.get_or_add_image_part(
                        "V:\\Dropbox\\elo-world\\video\\3d\\textures\\trainer_sprites\\"
                        + get_trainer_by_id(trainer.class_id, trainer.instance_id)[0][
                            "sprite"
                        ]
                    )
                    shape._element.blipFill.blip.rEmbed = rid
                    shape.shadow._element.getchildren()[3].getchildren()[
                        0
                    ].getchildren()[0].attrib["val"] = tier_colors[trainer.tier]
                    ...
                    image, rid = shape.part.get_or_add_image_part(
                        "V:\\Dropbox\\elo-world\\video\\3d\\textures\\trainer_sprites\\"
                        + get_trainer_by_id(
                            trainer.greatest_defeat.winning_trainer.class_id,
                            trainer.greatest_defeat.winning_trainer.instance_id,
                        )[0]["sprite"]
                    )
                    shape._element.blipFill.blip.rEmbed = rid
                    shape.shadow._element.getchildren()[3].getchildren()[
                        0
                    ].getchildren()[0].attrib["val"] = tier_colors[
                        trainer.greatest_defeat.winning_trainer.tier
                    ]
                elif shape.name == "gwpic":
                    image, rid = shape.part.get_or_add_image_part(
                        "V:\\Dropbox\\elo-world\\video\\3d\\textures\\trainer_sprites\\"
                        + get_trainer_by_id(
                            trainer.greatest_victory.losing_trainer.class_id,
                            trainer.greatest_victory.losing_trainer.instance_id,
                        )[0]["sprite"]
                    )
                    shape._element.blipFill.blip.rEmbed = rid
                    shape.shadow._element.getchildren()[3].getchildren()[
                        ...
                        trainer.greatest_victory.losing_trainer.tier
                    ]
                    ...
                    for p in shape.text_frame.paragraphs:
                        for r in p.runs:
                            if r.text == "t":
                                ...
                                r.text = (
                                    " " + str(abs(rank_diff)) if rank_diff < 0 else " "
                                )
