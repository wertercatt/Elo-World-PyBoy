...
def load_memory_map(path: str) -> Tuple[dict, dict]:
    ...
    return {int(key, 16): value for key, value in base.items()}, {
        value: int(key, 16) for key, value in base.items()
    }
...
def byte_to_pokestring(byte_array: Iterable[int]) -> str:
    return "".join(
        characters[b_int] if (b_int := int(b)) in characters else f"[0x{b:x}]"
        for b in byte_array
    )
...
def name_to_bytes(name: str, length: int = POKEMON_NAME_LENGTH) -> Iterable[int]:
    return (
        reverse_characters[name[i]] if i < len(name) else NAME_TERMINATOR
        for i in range(length)
    )


def load_trainer_info(
    trainer_id: int,
    trainer_index: int,
    lone_move_number: int,
    battle_save: str = BATTLE_SAVE,
...
def get_trainer_string(trainer_class: dict, trainer_instance: dict) -> str:
    ...
    return (
        f"a {trainer_class['class']} from {location if location != '' else 'somewhere'} who has a "
        + ", ".join(
            f"level {pokemon['level']} {pokemon['species']}"
            for pokemon in trainer_instance["party"]
        )
...
def set_value(
    target: bytearray, offset: int, source: Iterable[int], length: int
) -> None:
...
def copy_values(
    ...
    target[
        target_offset - GLOBAL_OFFSET : target_offset + length - GLOBAL_OFFSET
    ] = source[source_offset - GLOBAL_OFFSET : source_offset + length - GLOBAL_OFFSET]
...
def randomize_rdiv(source: bytearray, rng):
    source[DIVISOR_OFFSET : DIVISOR_OFFSET + 3] = (
...
def generate_demo(
    buttons: Iterable[int], buffer_button: int = B_BUTTON, buffer_size: int = 1000
) -> bytearray:
    return bytearray(
        [
...
def select_switch() -> bytearray:
    return generate_demo([UP_BUTTON, RIGHT_BUTTON, A_BUTTON])


def get_moves(battle_state: bytearray, offset: int) -> list:
    battle_moves = get_value(battle_state, offset, 4)
    return [moves[b] for b in battle_moves if b in moves.keys()]
...
def battle_x_as_y(
    ...
    if save_movie:
        ...
        bgb_options = [
            *bgb_options,
            ...
            "WavFileOut=1",
            "-set",
            f"RecordAVIfourCC={'cscd' if LOSSLESS else 'X264'}",
            "-set",
            "RecordHalfSpeed=0",
            "-set",
            "Speed=1",
    ...
    if auto_level:
        ...
        unleveled_enemy_mons = get_value(
            player_unleveled, ENEMY_PARTY_MONS, PARTY_STRUCT_SIZE * party_size
        )
            ...
            pprint(
                {
                    ...
                    "level": get_value(
                        player_unleveled, pokemon_offset + LEVEL_OFFSET, 1
                    )[0],
                    "hp": get_stat(player_unleveled, pokemon_offset + MAX_HP_OFFSET),
                    "attack": get_stat(
                        player_unleveled, pokemon_offset + ATTACK_OFFSET
                    ),
                    "moves": get_moves(
                        player_unleveled, pokemon_offset + MOVE_1_OFFSET
                    ),
                }
            )
    ...
    for i in range(party_size):
        ...
        set_value(
            base,
            PARTY_NICKNAMES + POKEMON_NAME_LENGTH * i,
            pokemon_name,
            POKEMON_NAME_LENGTH,
        )
        copy_values(
            ...
            ENEMY_TRAINER_NAME,
            base,
            PARTY_MON_OT + POKEMON_NAME_LENGTH * i,
            POKEMON_NAME_LENGTH,
        )
        if auto_level:
            # copy moves
            copy_values(
                player_unleveled,
                ENEMY_PARTY_MONS + (i * PARTY_STRUCT_SIZE) + MOVE_1_OFFSET,
                base,
                PARTY_MONS + (i * PARTY_STRUCT_SIZE) + MOVE_1_OFFSET,
                4,
            )
            # fix hp
            copy_values(
                base,
                PARTY_MONS + (i * PARTY_STRUCT_SIZE) + MAX_HP_OFFSET,
                base,
                PARTY_MONS + (i * PARTY_STRUCT_SIZE) + CURRENT_HP_OFFSET,
                2,
            )
    ...
    base_ai_action_count = (
        your_class["actionCount"] if "actionCount" in your_class else 3
    )
    ...
    if auto_level:
        ...
        for i in range(party_size):
            ...
            copy_values(
                battle_state,
                ENEMY_PARTY_MONS + (i * PARTY_STRUCT_SIZE) + MAX_HP_OFFSET,
                battle_state,
                ENEMY_PARTY_MONS + (i * PARTY_STRUCT_SIZE) + CURRENT_HP_OFFSET,
                2,
            )
    ...
    while True:
        ...
        if pc == PARTY_MENU_INIT_OFFSET:
            ...
            if using_item:
                button_sequence = choose_pokemon(
                    current_party_menu_choice, current_pokemon
                )
                using_item = False
            else:
                button_sequence = choose_pokemon(
                    current_party_menu_choice, get_pokemon_to_switch_to(battle_state)
                )
            ...
            using_item = False
            copy_values(
                battle_state, BATTLE_MON, ai_base, ENEMY_BATTLE_MON, BATTLE_MON_SIZE - 1
            )
            copy_values(
                battle_state, ENEMY_BATTLE_MON, ai_base, BATTLE_MON, BATTLE_MON_SIZE - 1
            )
            ...
            set_value(
                ai_base, TRAINER_CLASS_WITHOUT_OFFSET, [your_class["id"] - 200], 1
            )
            ...
            try:
                trainer_party_mon = [
                    get_party_mon(battle_state, PARTY_MONS, i)
                    for i in range(party_size)
                ]
                enemy_party_mon = [
                    get_party_mon(battle_state, ENEMY_PARTY_MONS, i)
            ...
            turn_summary = {
                ...
                "trainer_battle_mon": {
                    "species": get_string(
                        battle_state, BATTLE_MON_NAME, POKEMON_NAME_LENGTH
                    ).strip("@"),
                ...
                "enemy_battle_mon": {
                    "species": get_string(
                        battle_state, ENEMY_BATTLE_MON_NAME, POKEMON_NAME_LENGTH
                    ).strip("@"),
                    ...
                    "party_index": get_value(
                        battle_state, ENEMY_BATTLE_MON_PARTY_POS, 1
                    )[0],
                },
        ...
        set_value(
            battle_state,
            BATTLE_MON_PP,
            [0xFF] * battle_mon_move_count,
            battle_mon_move_count,
        )
    ...
    if save_movie:
        build_movie(
            ...
            output_dir,
            run_number,
            player_name=get_short_trainer_string(your_class, your_instance),
            enemy_name=get_short_trainer_string(enemy_class, enemy_instance),
            hash_id=seed if seed else "",
        )
...
def build_movie(
    movie_path, output_dir, run_number, player_name="", enemy_name="", hash_id=""
):
...
def run_one_battle(enemy, you, auto_level=True):
    ...
    battle_log = run_from_hashid(
        battle_hashid, save_movie=True, auto_level=auto_level, folder="for_video"
    )
...
def test_hash_id():
    for _ in range(200):
        ...
        battle_log_1 = battle_x_as_y(
            your_class,
            your_instance,
            enemy_class,
            enemy_instance,
            run_number=f"{hashid}_{battle_nonce}",
            save_movie=False,
            seed=hashid,
        )
...
def main():
    ...
    if not ROM_IMAGE.endswith("Pokemon - Red Version (UE) [S][!].gb"):
        print(
            'Sorry, but you need to make sure the ROM image is named "Pokemon - Red Version (UE) [S][!].gb". That\'s what BGB will look for'
        )
        quit_gracefully()
    ...
    with open(ROM_IMAGE, "rb") as rom:
        ...
        if hasher.hexdigest().upper() != "EA9BCAE617FDF159B045185467AE58B2E4A48B9A":
            print(
                "I found a file at "
                + ROM_IMAGE
                + ", but it doesn't look like it's the right version of Pokémon Red. Make sure you're getting a ROM with the SHA1 hash EA9BCAE617FDF159B045185467AE58B2E4A48B9A"
            )
            quit_gracefully()
    ...
    with open(BGB_PATH, "rb") as bgb:
        ...
        if hasher.hexdigest().upper() != "E07BCB5CA3B03E7BD854584148980E374082FFE0":
            print(
                "I found a file at "
                + BGB_PATH
                + ", but it doesn't look like it's the right version of BGB. Make sure you're getting BGB version 1.5.8 32 bit (which is not the newest version), and that you're pointing to the .exe file directly."
            )
            quit_gracefully()

    print(
        "Great, it seems to be there. If you run into weird issues, try deleting your bgb.ini file, that can cause some issues."
    )
    print()
    ...
    if not ffmpeg_path:
        print(
            "It doesn't look like you have ffmpeg installed and on your path. If you just installed it, you might need to restart Command Prompt. Google it if you need help"
        )
        quit_gracefully()
    print(
        "Fantasic, looks like you have ffmpeg installed, but I didn't bother checking the version or anything. If you run into issues, make sure your ffmpeg installation is working."
    )
    print()

    print(
        "I'm not checking for the CamStudio lossless codec (please don't ask me to read the Win32 documentation just for this dumb little script...). Make sure you have it installed or ffmpeg will complain."
    )
    print()

    print(
        f"You're scratch path is set to {WORKING_DIR_BASE} and your output path is set to {OUTPUT_BASE}. Make sure you're happy with those, or edit battle_x_as_y.py to change them"
    )
    print()
