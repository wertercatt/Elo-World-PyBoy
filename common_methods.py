...
from common_types import Battle, Trainer, Tuple
...
def save_pickle(
    pickle_loc: str, trainer_dict: Dict[str, Trainer], battle_dict: Dict[str, Battle]
):
    with open(pickle_loc, "wb") as f:
        f.write(
            zlib.compress(
                pickle.dumps(
                    (trainer_dict, battle_dict), protocol=pickle.HIGHEST_PROTOCOL
                ),
                level=9,
            )
        )
