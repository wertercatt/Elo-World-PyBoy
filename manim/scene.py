...
class EloOpeningExample(Scene):
    def construct(self):
        winner = ImageMobject(
            trainer_images + "Spr_RG_Bug_Catcher.png", scale_to_resolution=3840
        )
        winner.set_resampling_algorithm(RESAMPLING_ALGORITHMS["nearest"])
        winner.shift(LEFT + UP)

        loser = ImageMobject(
            trainer_images + "Spr_RG_Blue_1.png", scale_to_resolution=3840
        )
        ...
        absolute_difference = MathTex(r"d &= \lvert 834 - 625 \rvert\\ &= 209").shift(
            RIGHT * 2
        )
        ...
        p_win = MathTex(
            r"P_w &= \frac 1 {1 + 10^{-d/400}}\\ &= \frac 1 {1 + 10^{-209/400}}\\ &= 0.77"
        ).shift(RIGHT * 2 + DOWN)
...
class EloPipeline(Scene):
    def construct(self):
        trainer_image_files = [
            trainer_images + img
            for img in os.listdir(trainer_images)
            if img.endswith(".png")
        ][3:]
        ...
        for i, img in enumerate(trainer_image_files[:8]):
            self.add_trainer(
                img, i, trainer_mobjects, text_mobjects, group_mobjects, elo_values
            )
        ...
        self.play(
            AnimationGroup(
                *(FadeIn(t) for t in trainer_mobjects), lag_ratio=0.5, run_time=3
            ),
            AnimationGroup(
                *(Write(t) for t in text_mobjects), lag_ratio=0.5, run_time=3
            ),
            FadeIn(gary),
            Write(versus_text),
            Write(gary_elo),
        ...
        while last_index < len(trainer_image_files) - 7:
            ...
            winner = random.random() < (
                1 / (1 + 10 ** ((opponent_elo - gary_elo_value) / 400))
            )
            ...
            self.play(
                AnimationGroup(
                    FadeIn(win_text, shift=UP),
                    FadeIn(lose_text, shift=UP),
                    lag_ratio=0.5,
                    run_time=0.5,
                )
            )
            ...
            elo_diff_text = DecimalNumber(
                elo_diff, color=RED if elo_diff < 0 else GREEN, include_sign=True
            )
            ...
            self.add_trainer(
                trainer_image_files[last_index],
                7,
                trainer_mobjects,
                text_mobjects,
                group_mobjects,
    ...
    def add_trainer(
        self, img, i, trainer_mobjects, text_mobjects, group_mobjects, elo_values
    ):
...
class EloUpdateFormula(Scene):
    def construct(self):
        formula = MathTex(
            r" \mathrm {elo_{winner}} \stackrel{+}{=} { {{K}} \over {10^{ { \mathrm {elo_{winner} }  - \mathrm { elo_{\mathrm loser} } } \over {{400}} } + 1} }"
        )
...
class DogsAndCats(Scene):
    def construct(self):
        dog_table = DecimalTable(
            [[5.9, 12.3, 6.1, 10.6], [1.2, 0.9, 1.1, 0.8]],
            row_labels=[
                MathTex(r"\iffalse green \fi s", r" = {snout}:{body}").set_color_by_tex(
                    "green", GREEN
                ),
                MathTex(
                    r"\iffalse yellow \fi h", r" = {height}:{length}"
                ).set_color_by_tex("yellow", YELLOW),
            ],
        )
        ...
        dog_formula = (
            ...
            .set_color_by_tex("s", GREEN)
            .set_color_by_tex("h", YELLOW)
        )
...
class EloLogistic(Scene):
    def construct(self):
        ...
        line = Line(
            (
                number_mobjects[0].get_corner(UP + LEFT)
                + trainer_mobjects[0].get_corner(DOWN + LEFT)
            )
            / 2,
            (
                number_mobjects[-1].get_corner(UP + RIGHT)
                + trainer_mobjects[-1].get_corner(DOWN + RIGHT)
            )
            / 2,
            stroke_width=1,
        )
        ...
        self.play(
            AnimationGroup(
                Create(box1), number_mobjects[6].animate.set_value(1), lag_ratio=0.5
            )
        )

        self.play(
            AnimationGroup(
                Create(box2), number_mobjects[13].animate.set_value(-1), lag_ratio=0.5
            )
        )
        self.wait()
        ...
        footnote = Text(
            '*the first coefficient ends up representing a "home field advantage"'
        )
...
class TheMathZone(Scene):
    def construct(self):
        ...
        self.play(
            AnimationGroup(*(Write(t) for t in text_group), lag_ratio=0.75), run_time=6
        )
        self.wait()
