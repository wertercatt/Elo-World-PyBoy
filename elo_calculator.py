...
def main():
    ...
    pokemon_lorelei_drew_against = [
        p for b in lorelei_draws for p in b.enemy.party_mons
    ]
    ...
    karate_master_wins = [
        b
        for b in battle_list
        if b.winning_trainer.class_id == 224
        and b.winning_trainer.instance_id == 1
        and b.losing_trainer.class_id == 239
    ]
    lance_wins_against_oak = [
        b
        for b in battle_list
        if b.winning_trainer.class_id == 247
        and b.losing_trainer.class_id == 226
        and b.losing_trainer.instance_id == 1
    ]
    ...
    oddish_battles = [
        b
        for b in battle_list
        if b.enemy.party_mons[0].species == "ODDISH"
        and b.player.party_mons[0].species == "ODDISH"
        and "Draw" in b.winner
    ]
    ...
    for trainer in battle_trainers:
        if (
            ...
            and trainer.elo > kde_boundaries[current_kde_tier]
        ):
            current_kde_tier += 1
        if (
            current_diff_tier < len(greatest_diff_boundaries)
            and trainer.elo > greatest_diff_boundaries[current_diff_tier]
        ):
            current_diff_tier += 1
        if (
            current_human_tier < len(human_boundaries)
            and trainer.elo >= human_boundaries[current_human_tier] - 0.1
        ):
    ...
    battle_list.sort(
        key=lambda b: b.losing_trainer.elo - b.winning_trainer.elo, reverse=True
    )
    for upset in battle_list[:100]:
        print(
            "\t".join(
                (
                    upset.original_path,
                    upset.winning_trainer.identifier,
                    str(upset.winning_trainer.elo),
                    upset.losing_trainer.identifier,
                    str(upset.losing_trainer.elo),
    ...
    if DO_POWERPOINT:
        ...
        for trainer in trainer_list:
            ...
            trainer.greatest_victory = max(
                victories, key=lambda b: b.losing_trainer.elo
            )
