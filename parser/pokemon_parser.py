...
def parse(file):
    ...
    for line in file:
        ...
        if line.endswith(":"):
            ...
            print("New trainer")
            current_class["instances"].append(
                parse_trainer(tokens[1], current_location, current_class_index)
            )
            current_class_index += 1
    trainer_classes.append(current_class)
...
def parse_trainer(line, location, index):
    trainer = {"location": location, "index": index, "party": []}
    ...
    if tokens[0] == "$FF":
        ...
        for token in tokens[1:-1]:
            pokemon = {"species": token, "level": level}
            trainer["party"].append(pokemon)
    return trainer
