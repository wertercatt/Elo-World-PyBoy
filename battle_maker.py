...
from master_controller import json, os, trainer_list
...
class Trainer:
    ...
    def __eq__(self, o: object) -> bool:
        return (
            isinstance(o, Trainer)
            and self.class_id == o.class_id
...
def read_battle_json(path):
    ...
    return Matchup(
        Trainer(battle_summary["player_class"], battle_summary["player_id"]),
...
def get_list_of_battles():
    combatants = {
        Trainer(trainer[0]["id"], trainer[1]["index"]) for trainer in trainer_list
    }
...
def get_list_of_battles_that_were_to_be_done():
    with open("battles_to_run3.json") as f:
        battles_that_were_to_be_run = json.load(f)
    return {
        Matchup(
            Trainer(battle[0][0], battle[0][1]), Trainer(battle[-1][0], battle[-1][1])
        )
        for battle in battles_that_were_to_be_run
    }
...
def find_jsons(base_json_dir):
    jsons = [
        f"{base_json_dir}\\{f}"
        for f in os.listdir(base_json_dir)
        if f.endswith(".json")
    ]
    return jsons
...
def main():
    ...
    jsons = find_jsons(
        "V:\\Dropbox\\elo-world\\elo_world_pokemon_red_output\\json\\rival_hotfix"
    )
    print(len(jsons))
    json_battles = {
        read_battle_json(jsons[i]) for i in progressbar.progressbar(range(len(jsons)))
    }
    ...
    battle_dump = [
        [[combatant.class_id, combatant.instance_id] for combatant in battle.combatants]
